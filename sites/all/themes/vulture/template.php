...
function phptemplate_preprocess_page(&$vars) {
  ...
  // Add theming suggestions for page-nodetype.tpl.php files
  if ($vars['node']->type != "") {
    $vars['template_files'][] = "page-node-" . str_replace('_','-', $vars['node']->type);
  }
  
  // Add theming suggestions for page-arg(1)-arg(2)-tpl.php
...
/**
 * Add a registry function that allows the addition of other functions to retheme forms
 */
 
function vulture_theme(){
  /** Function edited out (see line 700)
  return array(
  ...
  */
  return array(
    // The form ID.
    'makemeeting_pollpanel_form' => array(
 ...
 /**
  * Remove the position and patrol fields from profile unless viewed by administrator (Currently breaks the website)
  */
  
/** function vulture_profile_node_form ($form){
     global $user;
     if (in_array('scout', $user->roles)){
       unset($form['group_profile_adult']);
     }
     if(!in_array('administrator', $user->roles)){
       unset($form['group_profile']['field_profile_position']);
       unset($form['group_profile']['field_profile_patrol']);
     }
...
/**  $form['signup_form_data']['Name'] = array(
  ...
  $form['signup_form_data']['Email'] = array(
    ...
    '#multiple' => FALSE,
    '#description' => t('Change your e-mail address if this isn\'t the one you would like to use.'),
  );
*/
 
  $form['signup_form_data']['Parent Coming'] = array(
    '#type' => 'select',
    '#title' => t('Parent Coming?'),
    '#options' => array(
      'No' => t('No'),
      'Yes' => t('Yes'),
    ),
    '#multiple' => FALSE,
    '#description' => t('Check this box if your parent is planning on coming'),
  );
